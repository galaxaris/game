...
from api.utils.Console import print_error, print_info, print_warning, print_success


def init_ressource_manager(ressource_manager: Resource, audio_manager: AudioManager, assets_dir: str,
                           assets_store: str = "assets\\assets_store.json") -> dict:
    """
...
def load_resources(glob: Resource, audio_manager: AudioManager, ressources_dic: dict, assets_path: str) -> dict:
    ...
    def _format_error(key, value):
        print_error(
            f"Invalid format for {key} in assets store. Expected a dictionary of key-path pairs, got {type(value)}. Skipping {key}.")
    ...
    def _not_list_error(key, key_type):
        print_error(
            f"Invalid format for {key} in {key_type} of assets store. Expected a list of 1 string and 2 integers, got {type(key)} of {len(key)} elements.")
        ...
        if key == "textures":
            ...
            try:
                for r_key, r_val in value.items():
                    ...
                    elif (len(r_val) != 3 or not isinstance(r_val[0], str) or not isinstance(r_val[1],
                                                                                             int) or not isinstance(
                            r_val[2], int)):
                        _not_list_error(r_key, key)
                    else:
                        loaded_ressources[key][r_key] = Animation(Texture(f"{assets_path}\\Images\\{r_val[0]}", glob),
                                                                  r_val[1], r_val[2])
